...
class MemoQServerClient:
    ...
    def lookup_terms(
        ...
        """
        Look up terminology for a list of segments against a Termbase.

        Args:
            tb_guid  : Termbase GUID.
            segments : Plain-text source segments (no XML wrapping needed).
            src_lang : Source language code (e.g. 'eng').
            tgt_lang : Target language code (e.g. 'bul').
        ...
        Returns:
            ...
            Only segments that have at least one hit are included.
        """
        payload: Dict = {
            "Segments": [f"<seg>{seg}</seg>" for seg in segments],
        }
        if src_lang:
            payload["SourceLanguage"] = src_lang
        if tgt_lang:
        ...
        try:
            logger.info(f"\U0001f4da TB LOOKUP REQUEST: guid={tb_guid}, segments={len(segments)}")
            result = self._make_request("POST", endpoint, data=payload)
            logger.debug(f"TB LOOKUP RESPONSE: {result}")

            # Response schema:
            #   {"Result": [ [TBHit, ...], [TBHit, ...], ... ]}
            #   One inner list per input segment.
            #   TBHit keys: SourceTerm, TargetTerm, MatchRate, IsForbidden,
            #               LengthInSegment, StartPosInSegment, Entry
            result_list = []
            ...
            for seg_idx, tb_hits in enumerate(result_list):
                if not tb_hits:
                    continue
                # tb_hits may be a list of TBHit dicts, or a single dict
                if isinstance(tb_hits, dict):
                    tb_hits = [tb_hits]
                ...
                for hit in tb_hits:
                    ...
                    hits.append({
                        "source":       source_term,
                        "target":       target_term,
                        "is_forbidden": bool(hit.get("IsForbidden", False)),
                        "match_rate":   int(hit.get("MatchRate", 100)),
                    })

                if hits:
                    normalized[seg_idx] = hits
                    logger.info(f"  Segment {seg_idx}: {len(hits)} term hit(s)")

            logger.info(f"TB lookup complete: {len(normalized)} segment(s) with hits")
            return normalized

        except Exception as e:
            logger.error(f"TB term lookup error: {e}", exc_info=True)
            return {}
